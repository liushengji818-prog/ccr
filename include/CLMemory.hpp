	...
	public:
		CLMemory(cl_context & clContext, cl_command_queue & clQueue, const cl_mem_flags flags, const size_t size, T * const pData)
		 : m_clQueue(clQueue), m_bFree(false), m_size(size), m_pData(pData) {
			 m_clMem = clCreateBuffer(clContext, flags, m_size, nullptr, nullptr);
		}
		...
		CLMemory(cl_context & clContext, cl_command_queue & clQueue, const cl_mem_flags flags, const size_t count, const bool noAllocation = false)
		 : m_clQueue(clQueue), m_bFree(true), m_size(sizeof(T) * count), m_pData(noAllocation ? nullptr : new T[count]) {
			m_clMem = clCreateBuffer(clContext, flags, m_size, nullptr, nullptr);
		}
		...
		static void setKernelArg(cl_kernel & clKernel, const cl_uint arg_index, const T & t) {
			const cl_int ret = clSetKernelArg(clKernel, arg_index, sizeof(T), static_cast<const void *>(&t));
			if (ret != CL_SUCCESS) {
		...
		void setKernelArg(cl_kernel & clKernel, const cl_uint arg_index) const {
			const cl_int ret = clSetKernelArg(clKernel, arg_index, sizeof(cl_mem), static_cast<const void *>(&m_clMem));
			if( ret != CL_SUCCESS ) {
				throw std::runtime_error("clSetKernelArg failed - " + toString(arg_index) + " - " + toString(ret));
			}
		}

		void read(const bool bBlock, cl_event * pEvent = nullptr) const {
			const cl_bool block = bBlock ? CL_TRUE : CL_FALSE;
			auto res = clEnqueueReadBuffer(m_clQueue, m_clMem, block, 0, m_size, m_pData, 0, nullptr, pEvent);
			if(res != CL_SUCCESS) {
		...
		void write(const bool bBlock) const {
			const cl_bool block = bBlock ? CL_TRUE : CL_FALSE;
			auto res = clEnqueueWriteBuffer(m_clQueue, m_clMem, block, 0, m_size, m_pData, 0, nullptr, nullptr);
			if( res != CL_SUCCESS ) {
