	...
	const uchar suffixCount)
{
	...
	// Fast suffix check optimization
	if (suffixCount > 0 && suffixCount <= 10) {
		ulong mod = 1;
		for (int k = 0; k < suffixCount; ++k) mod *= 58;

		ulong acc = 0;
		for (int k = 0; k < 25; ++k) {
			acc = (acc * 256 + tron_hash[k]) % mod;
		}

		char suffixChars[10];
		for (int k = 0; k < suffixCount; ++k) {
			suffixChars[suffixCount - 1 - k] = alphabet[acc % 58];
			acc /= 58;
		}
		...
		for (uint j = 0; j < matchingCount; ++j) {
			bool match = true;
			for (uint offset = 0; offset < suffixCount; ++offset) {
				const uint i = 34 - 1 - offset;
				const uint dataIndex = j * 34 + i;
				if ((suffixChars[suffixCount - 1 - offset] & data1[dataIndex]) != data2[dataIndex]) {
					match = false;
					break;
				}
			}
	...
	const uint addressLength = 34;
	uchar matchingHash[34];
	for (uint i = 0; i < addressLength; i++){
		matchingHash[i] = (uchar)tron_hash_address[i];
	}
	const uint prefixLimitGlobal = min((uint)prefixCount, addressLength);
	const uint suffixLimitGlobal = min((uint)suffixCount, addressLength);
	for(uint j = 0; j < matchingCount; j++) {
		uint scorePrefix = 0;
		uint scoreSuffix = 0;
		if(prefixCount > 0) {
			for (uint i = 0; i < prefixLimitGlobal; ++i) {
				const uint dataIndex = j * addressLength + i;
				if ((matchingHash[i] & data1[dataIndex]) == data2[dataIndex]) {
					++scorePrefix;
				} else {
					break;
				}
		...
		if(suffixCount > 0) {
			for (uint offset = 0; offset < suffixLimitGlobal; ++offset) {
				const uint i = addressLength - 1 - offset;
				const uint dataIndex = j * addressLength + i;
				if ((matchingHash[i] & data1[dataIndex]) == data2[dataIndex]) {
					++scoreSuffix;
				} else {
