...
static std::string hexToStr(const std::string &str)
{
	...
	for (size_t i = 0; i < str.length(); i += 2)
	{
		...
		// 改进: 添加错误检�?		if (errno != 0 || endptr == byte.c_str() || *endptr != '\0' || val < 0 || val > 255)
		{
...
unsigned int getKernelExecutionTimeMicros(cl_event &e)
{
	...
	clWaitForEvents(1, &e);
	clGetEventProfilingInfo(e, CL_PROFILING_COMMAND_START, sizeof(timeStart), &timeStart, nullptr);
	clGetEventProfilingInfo(e, CL_PROFILING_COMMAND_END, sizeof(timeEnd), &timeEnd, nullptr);
...
cl_command_queue Dispatcher::Device::createQueue(cl_context &clContext, cl_device_id &clDeviceId)
{
	...
	const cl_command_queue ret = clCreateCommandQueueWithProperties(clContext, clDeviceId, &p, nullptr);
#else
	const cl_command_queue ret = clCreateCommandQueue(clContext, clDeviceId, p, nullptr);
#endif
	return ret == nullptr ? throw std::runtime_error("failed to create command queue") : ret;
}

cl_kernel Dispatcher::Device::createKernel(cl_program &clProgram, const std::string s)
{
	cl_kernel ret = clCreateKernel(clProgram, s.c_str(), nullptr);
	return ret == nullptr ? throw std::runtime_error("failed to create kernel \"" + s + "\"") : ret;
}
...
Dispatcher::Device::Device(
	...
	: m_parent(parent),
	  ...
	  m_memData1(clContext, m_clQueue, CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY, 34 * mode.matchingCount),
	  m_memData2(clContext, m_clQueue, CL_MEM_READ_ONLY | CL_MEM_HOST_WRITE_ONLY, 34 * mode.matchingCount),
...
void Dispatcher::run()
{
	m_eventFinished = clCreateUserEvent(m_clContext, nullptr);
...
void Dispatcher::init()
{
	...
	{
		pInitEvents[i] = clCreateUserEvent(m_clContext, nullptr);
	...
	for (size_t i = 0; i < deviceCount; ++i)
	{
		m_vDevices[i]->m_eventFinished = nullptr;
...
{
	// Set mode data
	for (auto i = 0; i < m_mode.matchingCount * 34; ++i)
	{
	...
	// Note: seed will be set before each dispatch in dispatch() method
...
void Dispatcher::initContinue(Device &d)
{
	...
	if (sizeLeft)
	{
		// Set the seed for initialization (round 0)
		...
		const auto resEnqueue = clEnqueueNDRangeKernel(d.m_clQueue, d.m_kernelInit, 1, &d.m_sizeInitialized, &sizeRun, nullptr, 0, nullptr, &event);
...
void Dispatcher::enqueueKernelDevice(Device &d, cl_kernel &clKernel, size_t worksizeGlobal, cl_event *pEvent = nullptr)
{
	try
	{
...
void Dispatcher::enqueueKernel(cl_command_queue &clQueue, cl_kernel &clKernel, size_t worksizeGlobal, const size_t worksizeLocal, cl_event *pEvent = nullptr)
{
	...
	while (worksizeGlobal)
	{
		...
		const size_t *const pWorksizeLocal = (worksizeLocal == 0 ? nullptr : &worksizeLocal);
		const auto res = clEnqueueNDRangeKernel(clQueue, clKernel, 1, &worksizeOffset, &worksizeRun, pWorksizeLocal, 0, nullptr, pEvent);
...
{
	// Update seed for this round to ensure proper randomization across all 256 bits
	// The round number is distributed across all seed components to avoid fixed middle bits
	cl_ulong4 roundSeed;
	cl_ulong carry = 0;

	// Add round to seed.s[0]
	// Overflow detection: when C = A + B, overflow occurs if C < A (or equivalently, C < B)
	roundSeed.s[0] = d.m_clSeed.s[0] + d.m_round;
	carry = (roundSeed.s[0] < d.m_clSeed.s[0]) ? 1 : 0;

	// Propagate carry and mix round into seed.s[1]
	cl_ulong temp1 = d.m_clSeed.s[1] + carry;
	carry = (temp1 < d.m_clSeed.s[1]) ? 1 : 0;
	roundSeed.s[1] = temp1 + (d.m_round >> 32);
	carry += (roundSeed.s[1] < temp1) ? 1 : 0;

	// Propagate carry and mix round into seed.s[2]
	cl_ulong temp2 = d.m_clSeed.s[2] + carry;
	carry = (temp2 < d.m_clSeed.s[2]) ? 1 : 0;
	roundSeed.s[2] = temp2 + ((d.m_round >> 16) ^ (d.m_round << 16));
	carry += (roundSeed.s[2] < temp2) ? 1 : 0;

	// Propagate carry and mix round into seed.s[3]
	roundSeed.s[3] = d.m_clSeed.s[3] + carry + ((d.m_round << 32) ^ d.m_round);

	// Set the updated seed for this round
	CLMemory<cl_ulong4>::setKernelArg(d.m_kernelInit, 4, roundSeed);
	...
	const std::string & postUrl)
{
	...
	// Format private key - distribute round across all seed components to match dispatch logic
	// Overflow detection: when C = A + B, overflow occurs if C < A (or equivalently, C < B)
	cl_ulong carry = 0;
	cl_ulong4 seedRes;
	...
	carry = (seedRes.s[0] < seed.s[0]) ? 1 : 0;

	// Propagate carry and mix round into seed.s[1]
	cl_ulong temp1 = seed.s[1] + carry;
	carry = (temp1 < seed.s[1]) ? 1 : 0;
	seedRes.s[1] = temp1 + (round >> 32);
	carry += (seedRes.s[1] < temp1) ? 1 : 0;

	// Propagate carry and mix round into seed.s[2]
	cl_ulong temp2 = seed.s[2] + carry;
	carry = (temp2 < seed.s[2]) ? 1 : 0;
	seedRes.s[2] = temp2 + ((round >> 16) ^ (round << 16));
	carry += (seedRes.s[2] < temp2) ? 1 : 0;

	// Propagate carry and add foundId to seed.s[3]
	seedRes.s[3] = seed.s[3] + carry + ((round << 32) ^ round) + r.foundId;
...
void Dispatcher::onEvent(cl_event event, cl_int status, Device &d)
{
	...
	else if (d.m_eventFinished != nullptr)
	{
		initContinue(d);
	}
	else
